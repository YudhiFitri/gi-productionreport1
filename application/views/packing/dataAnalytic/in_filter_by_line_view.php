...
<body class="hold-transition sidebar-mini">
						...
						<div class="col-12">
							<div class="card card-info card-outline">
								<div class="card-header">
									<h3 class="card-title">Finish Good Show IN</h3>
								</div>
								<div class="card-body">
									<table class="table table-hover table-striped table-bordered tableCursor" style="width:100%" id="tableFGAllIn">
										<thead>
											<tr>
												<th>ID</th>
												<th>Lokasi</th>
										...
										<tbody>
											<?php foreach ($data as $d) : ?>
												<tr>
													...
													<td><?= $d->jmlPcs; ?></td>
													<td><?= $d->status; ?></td>
													<td><button class="btn btn-info btn-sm btnDetailIn">Detail</button></td>
												</tr>
											<?php endforeach; ?>
										</tbody>
										<tfoot>
	...
	<script>
		$(document).ready(function() {
			var tableFGAllIn = $('#tableFGAllIn').DataTable({
				dom: 'lBfrtip',
				lengthMenu: [
					[5, 10, 25, 50, 75, 100, -1],
			...
			$('#tableFGAllIn tbody').on('click', 'button.btnDetailIn', function() {
				let selectedRow = tableFGAllIn.row($(this).parents('tr')).data();
				open(`<?= site_url("TransferArea/ajax_show_detail_fg_by_lokasi_in"); ?>/${selectedRow[1]}`, '_self');
